...
const dateConfig = (app) => {
    ...
    app.command('/date-config', async ({ command, client, body, ack, respond, logger }) => {
        ...
        if (!registration) {
            ...
            respond(`I haven't been registered in this channel yet. Please add (or readd) <@${user_id}> to this channel to begin setting up dates for this channel. You can add me by clicking on me -> 'Add this app to a channel ...'.`)
            return
        }
        try {
            const { ok, channel: { name } } = await client.conversations.info({ channel: command.channel_id })      
    ...
    app.view('date-config_modal', async ({ ack, body, view, client, logger }) => {
        await ack()
        ...
        const acknowledged = await updateChannel(channelId, { pollingDay, pairingDay, frequency, endDate }, logger)
